...
internal sealed class Camera
{
    ...
    /// <summary>Zooms the orbit view.</summary>
    internal void Zoom(float notches)
    {
        _orbitDistance = Math.Clamp(
            _orbitDistance * MathF.Pow(1.18f, -notches), MinimumOrbit, MaximumOrbit);
    }
        ...
        Vector3 nose, Vector3 deck, Vector3 portOffset)
    {
        ...
        switch (_mode)
        {
            ...
            default:
            {
                float distance = _mode == Mode.Chase ? ChaseDistance : _orbitDistance;
                float lift = _mode == Mode.Chase ? ChaseLift : 0f;
    ...
    /// <summary>A one-line description, for the heads-up display.</summary>
    internal string Describe() => _mode switch
    {
        Mode.Chase => "CHASE",
        Mode.Orbit => $"ORBIT {_orbitDistance:F0} m",
