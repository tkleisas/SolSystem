"""
Sketch -- "The Modules", radial variant. A Workers waystation.
...
top of that order and never in front of it.
...
it works -- fifteen years, no spin ring, no regrets the logs record.
    ...
    blender --background --python tools/blender/stations/sketches/modules_b.py
"""
...
def can(col, parts, name, radius, location, rotation, length, key, windows=5):
    ...
    # End collars, on the can's own axis. The collar is a torus rotated to face
    # along the can: z-axis cans need none, x-axis a quarter turn about y.
    for end in (-1, 1):
        if rotation[1] != 0.0:      # along x
            collar_loc = (location[0] + end * (length / 2.0 - 0.5),
                          location[1], location[2])
            collar_rot = (0, math.pi / 2, 0)
        elif rotation[0] != 0.0:    # along y
...
def build_ferry(col, parts, z):
    """The parked ferry: a Soyuz ball and cone on the fore port."""
    ball = sphere("FerryBall", radius=2.8, location=(0, 0, z),
    ...
    cone = lathe("FerryCone", [
        ...
        (1.3, 4.5),
        (1.1, 5.6),
    ], segments=40, location=(0, 0, z + 1.6))
    assign(cone, "hull")
...
def build_node(col, parts):
    """The well the town grows around: a fat node with six ports."""
    node = lathe("Node", [
        (0.0, -6.0),
        (4.2, -5.0),
        (6.0, -3.0),
...
def build_radiators(col, parts):
    """Two radiator wings on outriggers off the aft can, Train DNA at small scale."""
    for side in (-1, 1):
        ...
        wing = box(f"Wing{side:+d}", (0.18, 20.0, 9.0),
                   location=(0, side * (5.8 + 10.0), -16.0))
        assign(wing, "radiator")
        link(wing, col)
        parts.append(wing)

        hot = box(f"WingHot{side:+d}", (0.05, 11.0, 9.0),
                  location=(0.12, side * (5.8 + 14.5), -16.0))
        assign(hot, "radiator_hot")
        link(hot, col)
        parts.append(hot)

        for j in range(1, 3):
            seam = box(f"WingSeam{side:+d}{j}", (0.3, 20.0, 0.3),
                       location=(0, side * (5.8 + 10.0), -16.0 - 4.5 + j * 3.0))
...
def build_station(col):
    ...
    # The axial cans: fore with the ferry, aft with the radiators.
    can(col, parts, "ForeCan", 3.4, (0, 0, 14.0), (0, 0, 0), 12.0, "hull_new",
        windows=5)
    build_ferry(col, parts, 22.5)

    can(col, parts, "AftCan", 4.0, (0, 0, -12.0), (0, 0, 0), 12.0, "hull",
        windows=4)
    build_radiators(col, parts)
...
def main():
    reset()
    col = collection("ModulesB")
    build_materials()

    build_station(col)

    print("  modules B  46 m cluster, 6-port node + 6 cans + ferry, no spin")

    render_views("/tmp/station-sketches/radial.png", SHOTS, resolution=900,
                 samples=28)

    # The maintenance story: the node, its rail, the radial collars, the farm.
    render_preview("/tmp/station-sketches/radial_detail.png",
                   target=(4.0, 0.0, 3.0), distance=42.0,
                   azimuth=35.0, elevation=14.0, resolution=900, samples=28)
